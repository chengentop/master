          ...
          <div v-for=" (menu,_index) in allPermission" :key="menu.menuname">
            ...
            <div style="display: inline-block;margin-left:20px;">
              <el-checkbox-group v-model="tempRole.permissions">
                ...
                >
                  <span
                    :class="{requiredpermission:perm.requiredpermission===1}"
...
export default {
  ...
  methods: {
    getAllPermisson() {
      //查询所有权限
      this.api({
        url: "/power/listAllPermission.json",
        method: "post"
      }).then(data => {
        this.allPermission = data.data.powers;
      });
    },
    getList() {
      //查询列表
      ...
      this.api({
        url: "/role/listRole.json",
        method: "post"
      }).then(data => {
        this.listLoading = false;
        this.list = data.data.roles;
      });
    },
    getIndex($index) {
      //表格序号
    ...
    showUpdate($index) {
      ...
      for (let i = 0; i < role.menus.length; i++) {
        ...
        for (let j = 0; j < perm.length; j++) {
          this.tempRole.permissions.push(perm[j].powerid);
        }
      }
    ...
    isMenuNone(_index) {
      ...
      for (let j = 0; j < menu.length; j++) {
        if (this.tempRole.permissions.indexOf(menu[j].powerid) > -1) {
          result = false;
          break;
        }
      }
    ...
    isMenuAll(_index) {
      ...
      let result = true;
      for (let j = 0; j < menu.length; j++) {
        if (this.tempRole.permissions.indexOf(menu[j].powerid) < 0) {
          result = false;
          break;
        }
      }
    ...
    allPerm(_index) {
      ...
      let menu = this.allPermission[_index].permissions;
      for (let j = 0; j < menu.length; j++) {
        this.addUnique(menu[j].powerid, this.tempRole.permissions);
      }
    },
    noPerm(_index) {
      //全部取消选中
      let menu = this.allPermission[_index].permissions;
      for (let j = 0; j < menu.length; j++) {
        let idIndex = this.tempRole.permissions.indexOf(menu[j].powerid);
        if (idIndex > -1) {
    ...
    checkRequired(_perm, _index) {
      //本方法会在勾选状态改变之后触发
      let powerid = _perm.powerid;
      if (this.tempRole.permissions.indexOf(powerid) > -1) {
        //选中事件
    ...
    makeReuqiredPermissionChecked(_index) {
      ...
      for (let j = 0; j < menu.length; j++) {
        ...
        if (perm.requiredpermission === 1) {
          //找到本菜单的必选权限,将其勾上
          this.addUnique(perm.powerid, this.tempRole.permissions);
        }
      }
    }
  }
